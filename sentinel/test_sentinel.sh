...
SLAVE_IP=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' redis-slave1)
SENTINEL_IP=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' sentinel1)
...
docker exec sentinel1 redis-cli -p 26379 SENTINEL get-master-addr-by-name rmaster
echo Current slaves are
docker exec sentinel1 redis-cli -p 26379 SENTINEL slaves rmaster
echo ------------------------------------------------
